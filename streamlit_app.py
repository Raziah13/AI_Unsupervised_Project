...
# Use triple quotes correctly - the key is to close the string properly
content = '''
    ...
    with tab3:
        ...
        if comparison_data:
            ...
            if 'kmeans_labels' in st.session_state:
                ...
                axes[0].set_title(f"K-Means - Silhouette: {st.session_state.kmeans_silhouette:.3f}")
            ...
            if 'dbscan_labels' in st.session_state:
                ...
                for k in unique_labels:
                    mask = st.session_state.dbscan_labels == k
                    if k == -1:
                        axes[1].scatter(X_pca[mask, 0], X_pca[mask, 1], c='black', s=50, alpha=0.6, label='Noise')
                    else:
                        axes[1].scatter(X_pca[mask, 0], X_pca[mask, 1], s=50, alpha=0.6)
                axes[1].set_title(f"DBSCAN - Clusters: {st.session_state.dbscan_n_clusters}")
    ...
    with tab5:
        ...
        if 'kmeans_labels' in st.session_state:
            ...
            **4. Next Steps**
            ...
            - Update clustering monthly with new data
            """)
