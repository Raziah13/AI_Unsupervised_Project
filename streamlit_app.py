# Create streamlit_app.py - Shows results only after clicking Run, with meaningful segment names
import os
...
with open('streamlit_app.py', 'w', encoding='utf-8') as f:
    ...
    f.write("            ax.set_title('K-Means Clustering Results (k=' + str(st.session_state.kmeans_k) + ')')\n")
    ...
    f.write("            \n")
    f.write("            # ============================================\n")
    f.write("            # SEGMENT SUMMARY WITH MEANINGFUL NAMES\n")
    f.write("            # ============================================\n")
    ...
    f.write("                \n")
    f.write("                # Determine gender percentage\n")
    ...
    f.write("                \n")
    f.write("                # Give meaningful names based on characteristics\n")
    f.write("                if avg_income > 70 and avg_spending > 60:\n")
    f.write("                    segment_name = 'VIP Premium Customers'\n")
    f.write("                    description = 'High income, high spending customers. They buy premium products and are not price sensitive.')\n")
    f.write("                    strategy = 'Offer exclusive products, early access to sales, VIP events, and premium loyalty program.')\n")
    f.write("                elif avg_income > 70 and avg_spending < 40:\n")
    f.write("                    segment_name = 'Smart Value Shoppers'\n")
    f.write("                    description = 'High income but careful spenders. They have money but look for good value.')\n")
    f.write("                    strategy = 'Focus on discounts, cashback offers, bundle deals, and price-match guarantees.')\n")
    f.write("                elif avg_income < 40 and avg_spending > 60:\n")
    f.write("                    segment_name = 'Aspiring Trendsetters'\n")
    f.write("                    description = 'Young customers with lower income but love to spend on trends and experiences.')\n")
    f.write("                    strategy = 'Use social media campaigns, influencer marketing, flash sales, and BNPL (Buy Now Pay Later) options.')\n")
    f.write("                elif avg_income < 40 and avg_spending < 40:\n")
    f.write("                    segment_name = 'Practical Frugal'\n")
    f.write("                    description = 'Budget-conscious customers who spend carefully on only essential items.')\n")
    f.write("                    strategy = 'Focus on essential products, basic value items, loyalty points for everyday purchases.')\n")
    f.write("                elif avg_age < 30 and avg_spending > 60:\n")
    f.write("                    segment_name = 'Young Trend Hunters'\n")
    f.write("                    description = 'Young customers who actively follow trends and spend on new products.')\n")
    f.write("                    strategy = 'Instagram, TikTok marketing, influencer partnerships, new arrivals, limited editions.')\n")
    f.write("                elif avg_age > 50 and avg_income > 60:\n")
    f.write("                    segment_name = 'Established Affluents'\n")
    f.write("                    description = 'Mature customers with stable high income. They value quality and service.')\n")
    f.write("                    strategy = 'Quality focus, service excellence, email newsletters, phone support, reliable products.')\n")
    f.write("                elif avg_age > 50 and avg_spending < 40:\n")
    f.write("                    segment_name = 'Comfort Keepers'\n")
    f.write("                    description = 'Senior customers who are cautious with spending and prefer familiar products.')\n")
    f.write("                    strategy = 'Trust and familiarity messaging, traditional media, direct mail, easy returns.')\n")
    f.write("                else:\n")
    f.write("                    segment_name = 'Regular Customers'\n")
    f.write("                    description = 'Balanced customers with moderate income and spending habits.')\n")
    f.write("                    strategy = 'Mix of offers, loyalty program, referral rewards, multi-channel approach.')\n")
    f.write("                \n")
    f.write("                segment_summary.append({\n")
    f.write("                    'Cluster Number': cluster,\n")
    f.write("                    'Segment Name': segment_name,\n")
    f.write("                    'Size': str(size) + ' (' + str(round(pct, 1)) + '%)',\n")
    f.write("                    'Avg Age': str(round(avg_age, 0)) + ' years',\n")
    f.write("                    'Avg Income': '$' + str(round(avg_income, 0)) + 'K',\n")
    f.write("                    'Avg Spending': str(round(avg_spending, 0)) + '/100',\n")
    f.write("                    'Gender': str(round(female_pct, 0)) + '% F / ' + str(round(male_pct, 0)) + '% M',\n")
    f.write("                    'What This Means': description,\n")
    f.write("                    'Marketing Strategy': strategy\n")
    f.write("                })\n")
    f.write("            \n")
    f.write("            # Display summary table\n")
    f.write("            summary_df = pd.DataFrame(segment_summary)\n")
    f.write("            st.dataframe(summary_df, use_container_width=True)\n")
    f.write("            \n")
    f.write("            # Key Findings\n")
    f.write("            st.subheader('Key Business Insights')\n")
    f.write("            \n")
    f.write("            largest = max(segment_summary, key=lambda x: int(x['Size'].split('(')[0].strip()))\n")
    f.write("            highest_spending = max(segment_summary, key=lambda x: int(x['Avg Spending'].split('/')[0]))\n")
    ...
    f.write("            kmeans_sil_str = str(round(st.session_state.kmeans_sil, 4)) if st.session_state.kmeans_sil is not None else 'N/A'\n")
    f.write("            dbscan_sil_str = str(round(st.session_state.dbscan_sil, 4)) if st.session_state.dbscan_sil is not None and st.session_state.dbscan_sil > 0 else 'N/A'\n")
    f.write("            \n")
    f.write("            comparison_data = {\n")
    f.write("                'Algorithm': ['K-Means', 'DBSCAN'],\n")
    f.write("                'Silhouette Score': [kmeans_sil_str, dbscan_sil_str],\n")
    ...
    f.write("            # YOUR ORIGINAL CODE - KEPT EXACTLY AS YOU HAD IT\n")
    ...
    f.write("                names = [s['name'][:12] for s in segment_details]\n")
    f.write("                ages = [s['age'] for s in segment_details]\n")
    f.write("                ax1.bar(names, ages, color='skyblue', edgecolor='black')\n")
    ...
    f.write("                ax2.bar(names, incomes, color='lightgreen', edgecolor='black')\n")
    ...
    f.write("                ax3.bar(names, spendings, color='salmon', edgecolor='black')\n")
    ...
    f.write("                ax4.pie(sizes, labels=labels, autopct='%1.1f%%', colors=colors[:len(sizes)], startangle=90)\n")
    ...
    f.write("                ax5.bar(x, female_data, width, label='Female', color='pink', edgecolor='black')\n")
    f.write("                ax5.bar(x, male_data, width, bottom=female_data, label='Male', color='lightblue', edgecolor='black')\n")
